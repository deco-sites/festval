...
function Card({ image, href, label }: Item) {
    return (<a href={href} class="flex flex-col items-center justify-center gap-2">
...
function CategoryGrid({ title, items }: Props) {
    ...
    return (<Section.Container class="custom-container realtive">
      <Section.Header title={title}/>

      {device === "desktop" ? (<div class="grid grid-cols-12 gap-5">
          {items.map((i) => (<Card {...i}/>))}
        </div>) : (<Slider class="carousel carousel-center sm:carousel-end gap-5 w-full">
