...
const Desktop = ({ navItems, logo, searchbar, loading, variant, icon, }: Props) => (<>
    <ModalSessionInit />
    <Modal id={SEARCHBAR_POPUP_ID}>
      <div class="absolute top-0 bg-base-100 container" style={{ marginTop: HEADER_HEIGHT_MOBILE }}>
        {loading === "lazy" ? (<div class="flex justify-center items-center">
    ...
    <div class="flex flex-col gap-4 pt-5 container border-b border-gray-300">
      <div class="flex justify-between items-center">
        ...
        <label for={SEARCHBAR_POPUP_ID} class="input input-bordered flex flex-1 justify-between items-center gap-2 w-full" aria-label="search icon button">
      ...
      <div class="flex items-center">
        <ul className="flex">
          {navItems?.slice(0, 10).map((item, index) => (<NavItem key={index} item={item}/>))}
        </ul>
...
const Mobile = ({ logo, searchbar, navItems, loading }: Props) => (<>
    <Drawer id={SEARCHBAR_DRAWER_ID} aside={<Drawer.Aside title="Search" drawer={SEARCHBAR_DRAWER_ID}>
          <div class="w-screen overflow-y-auto">
            {loading === "lazy" ? (<div class="h-full w-full flex items-center justify-center">
                <span class="loading loading-spinner"/>
              </div>) : (<Searchbar {...searchbar}/>)}
          </div>
        </Drawer.Aside>}/>
    <Drawer id={SIDEMENU_DRAWER_ID} aside={<Drawer.Aside title="Menu" drawer={SIDEMENU_DRAWER_ID}>
          {loading === "lazy" ? (<div id={SIDEMENU_CONTAINER_ID} class="h-full flex items-center justify-center" style={{ minWidth: "100vw" }}>
              <span class="loading loading-spinner"/>
            </div>) : (<Menu navItems={navItems ?? []}/>)}
        </Drawer.Aside>}/>
